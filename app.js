...
computeIRTBtn.addEventListener('click', () => {
  ...
  // exclude items with a single observed category (no information)
  ...
  if (!pairs.length) {
    alert(
      'All predicted items have a single category only. IRT cannot be estimated. ' +
...
function runIRT_JML_fromPairs_poly(pairs, {source}={}){
  ...
  items.forEach((it)=>{
    const m = stepsByItem[it];
    const deltas = (modelType==='pcm') ? delta_pcm[it] : delta_rsm;
    let cum=0;
    for(let k=0;k<m;k++){
      cum += deltas[k]||0;
