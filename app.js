...
function buildFeatureCards(headers){
  ...
  headers.forEach(h=>{
    ...
    card.querySelectorAll(`input[name="enc_${h.replace(/([\\[\\].])/g,'\\$1')}"]`).forEach(r=>{
...
function runIRT_JML_fromPairs_poly(pairs){
  ...
  items.forEach(it=>{
    ...
    stepsByItem[it] = Math.max(1, maxScore); // 至少二分类
  });
  ...
  items.forEach(it=>{
    ...
    const tZ=0, pVal=(z)=>2*(1-phiCdf(Math.abs(z))); // 占位展示
    itemsRows.push({
      Item:it,
